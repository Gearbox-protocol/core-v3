...
contract CreditFacadeV3UnitTest is TestHelper, BalanceHelper, ICreditFacadeV3Events {
    ...
    function test_U_FA_08_openCreditAccount_reverts_if_out_of_limits(uint128 a, uint128 b) public notExpirableCase {
        ...
        uint8 multiplier = uint8((maxDebt % 253) + 2);

        vm.assume(maxDebt < type(uint128).max / 256);

        vm.prank(CONFIGURATOR);
        creditFacade.setDebtLimits(minDebt, maxDebt, multiplier);

        if (minDebt != 1) {
            creditManagerMock.setManageDebt(minDebt - 1, 0, 0);
            ...
            creditFacade.openCreditAccount({
                ...
                calls: MultiCallBuilder.build(
                    MultiCall({
                        target: address(creditFacade),
                        callData: abi.encodeCall(ICreditFacadeV3Multicall.increaseDebt, (minDebt - 1))
                    })
                    ),
                referralCode: 0
            });
        ...
        creditManagerMock.setManageDebt(maxDebt + 1, 0, 0);
        ...
        creditManagerMock.setManageDebt(minDebt, 0, 0);
        creditFacade.setLastBlockBorrowed(uint64(block.number));
