...
contract PhantomTokenWithdrawerMock is IAdapter, IPhantomTokenAdapter {
    uint256 public constant override version = 3_10;
    ...
    function depositPhantomToken(address, uint256 amount) external override returns (bool) {
        address creditAccount = ICreditManagerV3(creditManager).getActiveCreditAccountOrRevert();
        ERC20Mock(depositedToken).burn(creditAccount, amount);
        PhantomTokenMock(phantomToken).mint(
            creditAccount, amount * 1e18 / PhantomTokenMock(phantomToken).exchangeRate()
        );
        return false;
    }
}
