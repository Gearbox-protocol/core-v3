...
contract CreditConfiguratorV3 is ICreditConfiguratorV3, ACLTrait, SanityCheckTrait {
    ...
    /// @notice Forbids opening new credit accounts. Can only be called by pausable admin.
    function forbidBorrowing() external override pausableAdminsOnly {
        CreditFacadeV3(creditFacade()).setBorrowingForbidden(true);
    }

    /// @notice Allows opening new credit accounts again. Can only be called by configurator.
    function allowBorrowing() external override configuratorOnly {
        CreditFacadeV3(creditFacade()).setBorrowingForbidden(false);
    }
