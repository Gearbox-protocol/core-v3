...
contract LinearInterestRateModelV3 is ILinearInterestRateModelV3 {
    ...
    /// @dev The first slope change point (obtuse -> intermediate region)
    uint256 internal immutable U_1_WAD;

    /// @dev The second slope change point (intermediate -> steep region)
    uint256 internal immutable U_2_WAD;

    /// @dev Base interest rate in RAY format
    uint256 internal immutable R_base_RAY;

    /// @dev Slope of the obtuse region
    uint256 internal immutable R_slope1_RAY;

    /// @dev Slope of the intermediate region
    uint256 internal immutable R_slope2_RAY;

    /// @dev Slope of the steep region
        ...
        returns (uint256)
    {
        if (isBorrowingMoreU2Forbidden && expectedLiquidity != 0) {
            ...
            return availableLiquidity > minAvailableLiquidity ? availableLiquidity - minAvailableLiquidity : 0; // U:[LIM-3]
        } else {
