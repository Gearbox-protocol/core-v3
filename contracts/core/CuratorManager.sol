...
import {PriceOracleV3} from "./PriceOracleV3.sol";
...
contract RiskConfigurator is Ownable2Step {
    ...
    function setPriceFeedFromStore(address pool, address token, address priceFeed, bool trusted) external onlyOwner {
        ...
        PriceOracleV3(priceOracles[pool]).setPriceFeed(
    ...
    function setReservePriceFeedFromStore(address pool, address token, address priceFeed) external onlyOwner {
        ...
        PriceOracleV3(priceOracles[pool]).setReservePriceFeed(
    ...
    function changePriceOracle(
        ...
        unchecked {
            for (uint256 i; i < len; ++i) {
                // IPriceOracleV3(newPriceOracle).setPriceFeed()
            }
        }
    ...
    // function addAdapter(uint256 version, address target, address creditManager) external onlyOwner {}
